...
/*! \brief Use this macro for alloc_style parameter if container doesn't
    allocate memory for its type.
*/

#define CAG_NO_ALLOC_STYLE(to, from, alloc_func, free_code) to = from

/*! \brief Use this macro for alloc_func parameter when container doesn't
    allocate memory for its type.
*/
...
/*! \brief Generic *linear_search* function declaration and definition, but used
    with comparison functions whose comparators are by address not by value.
*/
...
/* \brief Private Lomuto partitioning algorithm that can be used by
   Quicksort.
*/
...
/* \brief Private macros used to make a spurious typedef at the end of
   definitions so that CAG_DEF macros need a semi-colon after them when users
   invoke them.
*/
